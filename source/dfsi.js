...
export async function dfsi(canvas, matrix, initial_state) {
    ...
    while (true) {
        ...
        if (result === true) {
            ...
            dfs_output_html.value = "svvsd"

            return true; // Se encontró el objetivo
        } else if (result === false) {
            depthLimit++; // No se encontró el objetivo
        }
    ...
    while (stack.length > 0) {
        ...
        if (!visited_nodes.has(JSON.stringify(node))) {
            ...
            if (check_end(matrix, node)) {
                ...
                dfsi_output_html.value = "oasngvaouinvgoadn"
                return true, visited_nodes, next_stack;
            }
            ...
            push_orderly(stack, next_stack, neighbors, path, deep, depthLimit)
...
function push_orderly(stack, next_stack, neighbors, path, deep, depthLimit) {
